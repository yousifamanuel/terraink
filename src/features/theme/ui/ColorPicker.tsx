...
export default function ColorPicker({
  ...
  function scheduleSliderCommit(nextHex: string) {
  ...
  function commitColorImmediately(nextHex: string) {
    const normalized = normalizeHexColor(nextHex);
    if (!normalized || normalized === latestCommittedHexRef.current) return;
    clearSliderCommitTimer();
    onChange(normalized);
  }
  ...
  useEffect(() => {
    if (additionalList.length === 0) setShowMoreColors(false);
  }, [additionalList]);
  ...
  function updateCustomHsl(
    nextHsl: { h: number; s: number; l: number },
    commitMode: "none" | "deferred" | "immediate" = "none",
  ) {
    ...
    if (commitMode === "deferred") scheduleSliderCommit(nextHex);
    else if (commitMode === "immediate") commitColorImmediately(nextHex);
  }

  function handleHueChange(event: React.ChangeEvent<HTMLInputElement>) {
    const hue = clamp(Number(event.target.value) / 360, 0, 1);
    updateCustomHsl({ ...customHsl, h: hue }, "deferred");
  }

  function handleShadeChange(event: React.ChangeEvent<HTMLInputElement>) {
    const lightness = clamp(1 - Number(event.target.value) / 100, 0, 1);
    updateCustomHsl({ ...customHsl, l: lightness }, "deferred");
  }

  function handleRgbInput(channel: "r" | "g" | "b", rawValue: string) {
    const nextRgb = { ...rgbInput, [channel]: rawValue };
    setRgbInput(nextRgb);

    const values = [Number(nextRgb.r), Number(nextRgb.g), Number(nextRgb.b)];
    if (!values.every((v) => Number.isFinite(v) && v >= 0 && v <= 255)) return;
  ...
  function updateFromPreviewPointer(
    clientX: number,
    clientY: number,
    commitMode: "deferred" | "immediate" = "deferred",
  ) {
    ...
    updateCustomHsl({ ...customHsl, s: saturation, l: lightness }, commitMode);
  }

  function handlePreviewPointerDown(event: React.PointerEvent) {
  ...
  function handlePreviewPointerMove(event: React.PointerEvent) {
    if (!isPreviewDraggingRef.current) return;
    updateFromPreviewPointer(event.clientX, event.clientY, "deferred");
  }

  function handlePreviewPointerUp(event: React.PointerEvent) {
    if (!isPreviewDraggingRef.current) return;
